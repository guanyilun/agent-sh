...
export default function activate(ctx: ExtensionContext): void {
  ...
  // ── agent-sh events → Claude Code ────────────────────────────
  bus.on("agent:submit", async ({ query: userQuery, modeInstruction }) => {
    ...
    try {
      ...
      for await (const message of activeQuery) {
        switch (message.type) {
          ...
          case "assistant": {
            // Complete assistant message — only emit text if not already streamed
  ...
  bus.on("agent:reset-session", () => {
    // Each query() call is a new session — nothing to reset
  });
