...
import { InMemoryLiveView } from "../../../../src/agent/live-view.js";
import {
  parseTurns,
  inferPriority,
  slimTurn,
  ...
  makeCaptureHandler,
  type SummaryCtx,
} from "../../../../src/agent/extensions/summary-strategy/strategy.js";
import { InMemoryStore } from "../../../../src/agent/store.js";
import type { Store } from "../../../../src/agent/store.js";
import type { AgentShMessage } from "../../../../src/utils/llm-client.js";
...
function buildState(msgs: AgentShMessage[]): InMemoryLiveView {
  ...
  const state = new InMemoryLiveView(null, "test-iid");
  state.replaceMessages(msgs);
  return state;
}

function makeCtx(state: InMemoryLiveView, store: Store): SummaryCtx {
  return {
    liveView: state,
    store: () => store,
    bus: { on: () => {} },
