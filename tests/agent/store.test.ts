...
import { StoreRegistry } from "../../src/agent/store-registry.js";
...
describe("StoreRegistry", () => {
  test("get throws when name not registered", () => {
    const reg = new StoreRegistry();
    assert.throws(() => reg.get("missing"), /no store registered/);
  });

  test("register + get round-trips", () => {
    const reg = new StoreRegistry();
    const store = new InMemoryStore();
    reg.register("summary", store);
    assert.equal(reg.get("summary"), store);
  });

  test("register throws on conflict (no silent overwrite)", () => {
    const reg = new StoreRegistry();
    reg.register("summary", new InMemoryStore());
    assert.throws(
      () => reg.register("summary", new InMemoryStore()),
      /already registered/,
    );
  ...
  test("has reflects registrations", () => {
    const reg = new StoreRegistry();
    assert.equal(reg.has("x"), false);
    reg.register("x", new InMemoryStore());
    assert.equal(reg.has("x"), true);
  });

  test("names lists all registered", () => {
    const reg = new StoreRegistry();
    reg.register("a", new InMemoryStore());
    reg.register("b", new InMemoryStore());
