...
test("claudeStyle gutter: no pipe, sigil hugs the code, line number leads", () => {
  const lines = renderDiff(computeDiff(old, neu), {
    width: 60, filePath: "x.py", mode: "unified", claudeStyle: true,
  }).map(strip);
  ...
  assert.ok(!lines.some((l) => l.includes("│")), "no box pipe in claude style");
});

test("classic style keeps the │ gutter (pi-tui look unchanged)", () => {
  const lines = renderDiff(computeDiff(old, neu), {
    width: 60, filePath: "x.py", mode: "unified",
  }).map(strip);
  assert.ok(lines.some((l) => l.includes("│")), "classic style has the pipe gutter");
});
