...
| **You get** | `ExtensionContext` — substrate (bus, handlers, lifecycle) + slash-command sugar + nested `ctx.agent` (LLM, tools, instructions) and `ctx.shell` (compositor, palette, transforms) surfaces | `AgentShellCore` — bus, `query()`, lifecycle control (`activateBackend`, `kill`) |
...
// advisors via ctx.agent.adviseTool (see examples/extensions/interactive-prompts.ts).
